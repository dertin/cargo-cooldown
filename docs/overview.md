...
2. read Cargo metadata
3. inspect reachable registry packages
4. replace fresh versions with older compatible versions when possible
5. ask Cargo to validate the resulting lockfile
6. run the requested Cargo command when the graph is acceptable

For `cargo cooldown update`:

1. snapshot the current `Cargo.lock`
2. run `cargo update`
3. cool the updated lockfile
4. restore the original lockfile if `strict` mode fails
...
only warn about fresh versions that could not be cooled.

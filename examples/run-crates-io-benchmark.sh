...
SAMPLES=${SAMPLES:-3}
COOLDOWN_MINUTES_VALUE=${COOLDOWN_MINUTES:-131401}
COOLDOWN_NOW_VALUE=${COOLDOWN_NOW:-$(date -u +"%Y-%m-%dT%H:%M:%SZ")}
COOLDOWN_ENFORCEMENT_VALUE=${COOLDOWN_ENFORCEMENT:-strict}
COOLDOWN_CARGO_COMPATIBLE_ACCEPT_VALUE=${COOLDOWN_CARGO_COMPATIBLE_ACCEPT:-auto}
...
run_cooldown_update() {
  ...
  if [[ "${BENCH_VERBOSE}" == "1" ]]; then
    if ! (
      ...
      export COOLDOWN_MINUTES="${COOLDOWN_MINUTES_VALUE}"
      export COOLDOWN_ENFORCEMENT="${COOLDOWN_ENFORCEMENT_VALUE}"
      export COOLDOWN_CARGO_COMPATIBLE_ACCEPT="${COOLDOWN_CARGO_COMPATIBLE_ACCEPT_VALUE}"
...
Benchmark complete
  ...
  samples: ${SAMPLES}
  cooldown_minutes: ${COOLDOWN_MINUTES_VALUE}
  enforcement: ${COOLDOWN_ENFORCEMENT_VALUE}
  cargo_compatible_accept: ${COOLDOWN_CARGO_COMPATIBLE_ACCEPT_VALUE}
  lockfile_baseline: ${LOCKFILE_BASELINE_VALUE}

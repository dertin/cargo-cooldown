...
COOLDOWN_MODE_VALUE=${COOLDOWN_MODE:-strict}
LOCKFILE_POLICY_VALUE=${COOLDOWN_LOCKFILE_POLICY:-all}
...
run_cooldown_update() {
  ...
  if [[ "${BENCH_VERBOSE}" == "1" ]]; then
    if ! (
      ...
      export COOLDOWN_MODE="${COOLDOWN_MODE_VALUE}"
      export COOLDOWN_LOCKFILE_POLICY="${LOCKFILE_POLICY_VALUE}"
...
Benchmark complete
  ...
  mode: ${COOLDOWN_MODE_VALUE}
  lockfile_policy: ${LOCKFILE_POLICY_VALUE}
